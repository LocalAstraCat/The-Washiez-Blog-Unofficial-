...
import { createDraft, deleteDraft, fetchMyApplication, fetchMyOwnerFeedback, fetchMyPosts, PostInput, publishDraft, submitWriterApplication, updateDraft, useSupabaseQuery } from "@/lib/supabase";
import { CheckCircle2, Eye, FilePlus2, FileText, LogIn, MessageSquareText, PencilLine, Send, ShieldAlert, Trash2 } from "lucide-react";
...
export function OwnerFeedbackNotice({ body }: { body: string }) { return <aside className="owner-feedback-notice"><div><span className="section-kicker"><MessageSquareText size={14} /> Private owner note</span><p>Only you and the site owner can see this feedback.</p></div><div className="article-prose"><Streamdown>{body}</Streamdown></div></aside>; }
...
function WriterDesk() {
  ...
  const ownerFeedback = useSupabaseQuery(fetchMyOwnerFeedback, []);
  ...
  const selectedFeedback = useMemo(() => ownerFeedback.data?.find((feedback) => feedback.postId === selected?.id), [ownerFeedback.data, selected?.id]);
